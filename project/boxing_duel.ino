/*
 * ============================================================================
 *  boxing_duel.ino  —  Two-node real-time boxing duel on MKR WiFi 1010
 ...
 *  (a compile-time flag) instead of maintaining two near-duplicate .ino files.
 *
 *  PIPELINE (per node):
 *    FSR --analogRead--> EMA filter --> hit state machine (debounce + refractory)
...
void setup() {
  ...
  for (int i = 0; i < 4; ++i) {
    long sum = 0;
    for (int j = 0; j < 32; ++j) { sum += analogRead(FSR_PIN[i]); delay(2); }
